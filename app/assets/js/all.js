...
$(function () {
    ...
    if (location.href.includes('?')) {        
        let get_params = location.href.split('?')[1];
        let params_arr = []; //參數key-value
        let params_name = []; //參數名稱(key)
        let params_value = []; //參數值(value)
...
function search_product(params_name,params_value){
    ...
    let keyword = decodeURI(params_value[0]);
    let all_product = [];
    let searched_product = []; //選到的產品
    let unsearched_product = []; //未選到的產品

    const tastyItems = $(".tasty-item-block");
    ...
    // 產品頁篩選
    searched_product.forEach(element => {
        if ($("#"+element).hasClass('d-none')) {
            $("#"+element).removeClass('d-none');
            }            
    });
    unsearched_product.forEach(element => {
        if (!($("#"+element).hasClass('d-none'))) {
